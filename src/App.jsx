...
import { Calendar, Send, History, TrendingUp, Package, Circle } from 'lucide-react';
...
const App = () => {
  ...
  const calculateReport = () => {
    ...
    const nhapKhi = parseFloat(formData.nhapKhi) || 0;
...
const saveReport = () => {
  ...
  try {
    ...
    // Lưu tồn kho
    localStorage.setItem('last-inventory', JSON.stringify({
  ...
  const getMonthlyStats = () => {
    ...
    const totalBan = monthReports.reduce((sum, r) => sum + r.calculated.xacBan, 0);
    const totalNhap = monthReports.reduce((sum, r) => sum + r.calculated.xacNhap, 0);
    const totalBinhMoi = monthReports.reduce((sum, r) => sum + r.calculated.binhMoi, 0);
    const totalNhapKhi = monthReports.reduce((sum, r) => sum + r.calculated.nhapKhi, 0);
        ...
        {!showHistory ? (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 backdrop-blur rounded-xl p-5 space-y-4">
              <h3 className="font-bold text-lg mb-3 text-blue-300">📊 Thông tin bóng</h3>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-slate-300">Nhập tại quầy</label>
                  <input
                    type="number"
                    value={formData.nhapTaiQuay}
                    onChange={(e) => handleInputChange('nhapTaiQuay', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('tonHomQua', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
                <div>
                  <label className="text-sm text-slate-300">Tồn hôm nay *</label>
                  <input
                    type="number"
                    value={formData.tonHomNay}
                    onChange={(e) => handleInputChange('tonHomNay', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('doiHomNay', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
              ...
              <h3 className="font-bold text-lg mb-3 text-blue-300 pt-3">🔧 Thông tin bình</h3>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-slate-300">Vỏ bình hôm nay</label>
                  <input
                    type="number"
                    value={formData.voBinhHomNay}
                    onChange={(e) => handleInputChange('voBinhHomNay', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
                <div>
                  <label className="text-sm text-slate-300">Bình dở hôm nay</label>
                  <input
                    type="number"
                    value={formData.binhDoHomNay}
                    onChange={(e) => handleInputChange('binhDoHomNay', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
                <div>
                  <label className="text-sm text-slate-300">Bình tồn hôm qua</label>
                  <input
                    type="number"
                    value={formData.binhTonHomQua}
                    onChange={(e) => handleInputChange('binhTonHomQua', e.target.value)}
                    className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="0"
                  />
                </div>
              ...
              <h3 className="font-bold text-lg mb-3 text-blue-300 pt-3">💨 Nhập khí</h3>
              
              <div>
                <label className="text-sm text-slate-300">Số cân (kg)</label>
                <input
                  type="number"
                  step="0.1"
                  value={formData.nhapKhi}
                  onChange={(e) => handleInputChange('nhapKhi', e.target.value)}
                  className="w-full bg-slate-700 rounded-lg px-3 py-2 mt-1 focus:ring-2 focus:ring-blue-500 outline-none"
                  placeholder="0.0"
                />
              </div>
            ...
            {/* Preview báo cáo */}
            <div className="bg-gradient-to-br from-blue-900/50 to-purple-900/50 backdrop-blur rounded-xl p-5 border border-blue-500/30">
          ...
          <div className="space-y-4">
            {/* Monthly Report */}
            <div className="bg-slate-800/50 backdrop-blur rounded-xl p-4">
              <label className="text-sm font-semibold mb-2 block">Chọn tháng xem báo cáo</label>
              <input
                type="month"
                value={selectedMonth}
            ...
            {monthlyStats && (
              <div className="bg-gradient-to-br from-purple-900/50 to-blue-900/50 backdrop-blur rounded-xl p-5 border border-purple-500/30">
                <h3 className="font-bold text-xl mb-4">📈 Báo cáo tháng {selectedMonth}</h3>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">Số ngày</div>
                    <div className="text-2xl font-bold">{monthlyStats.soNgay}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">TB Bán/ngày</div>
                    <div className="text-2xl font-bold text-green-400">{monthlyStats.trungBinhBan}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">Tổng bán</div>
                    <div className="text-xl font-bold text-blue-400">{monthlyStats.totalBan}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">Tổng nhập</div>
                    <div className="text-xl font-bold text-purple-400">{monthlyStats.totalNhap}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">Bình mới</div>
                    <div className="text-xl font-bold text-yellow-400">{monthlyStats.totalBinhMoi}</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <div className="text-slate-400">Nhập khí (kg)</div>
                    <div className="text-xl font-bold text-cyan-400">{monthlyStats.totalNhapKhi.toFixed(1)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              {reports.length === 0 ? (
                ...
                reports.map((report, idx) => (
                  <div key={idx} className="bg-slate-800/50 backdrop-blur rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-blue-300">{formatDate(report.date)}</span>
